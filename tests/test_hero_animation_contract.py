...
* the copy is real DOM text that reads "BuildUrgent AI. Ship AI agents that do
  real work." and its
...
VISIBLE_COPY = "BuildUrgent AI. Ship AI agents that do real work."
...
def test_headline_reads_the_product_value_proposition():
    """The settled, visible copy must read the product value proposition."""
    ...
    assert "BuildUrgent AI" in visible
...
def test_headline_keeps_the_centred_three_line_block():
    """The authored line structure survives as three centred lines.
...
def test_style_css_has_no_hero_text_animation_left():
    ...
    # NB: only the two names that genuinely belonged to the removed hero-TEXT
    # layers. `hero-pulse` was never a hero-glyph animation - it was reused as the
    # live status-dot pulse, since renamed `badge-pulse` (see .hero-badge i).
    for name in ("hero-3d", "typing3d"):
